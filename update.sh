#!/bin/zsh
# 캉다방 통계 자동 업데이트 — git pull(코드 동기화) → collect.py(채널별 수집) → build_dashboard.py → site/<path>/ 복사 → Vercel 배포
...

{
  ...
  # 2) 빈 데이터 가드 — 주 채널(캉다방, data/kang/)의 오늘 포스트 CSV가 실제로 생겼는지.
  #    보조 채널(data/<key>/)은 collect.py 가 실패해도 옛 데이터로 계속 배포한다.
  TODAY_CSV="data/kang/channel_posts_$(date -u +%Y-%m-%d).csv"
  if [ ! -s "$TODAY_CSV" ]; then
  ...
  # 3) 빌드(채널별 data/<key>/dashboard.html) — 실패 시 배포 중단
